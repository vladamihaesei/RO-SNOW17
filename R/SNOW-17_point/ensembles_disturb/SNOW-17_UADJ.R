...
tab <- tab%>%filter(year(DAT)>2003 & year(DAT) < 2008)
tab.f <- tab%>%drop_na(c(TMED))%>%left_join(statii)
tab.f$TMED <- as.numeric(tab.f$TMED)
tab.f$PRECIP <- as.numeric(tab.f$PRECIP)
tab.f$ECHIVZAP <- as.numeric(tab.f$ECHIVZAP)
tab.f$GROSZ <- as.numeric(tab.f$GROSZ)
tab.f$DENSZAP <- as.numeric(tab.f$DENSZAP)
tab.f$PRECIP[is.na(tab.f$PRECIP)] <- 0
tab.f1 <- tab.f%>%filter(COD == cods[1])## Vf. Omu


#####
ani <- unique(year(tab.f1$DAT))
tab.f10 <- NULL

for (j in 1:(length(ani) - 1)) {
  
  t3 <- tab.f1 %>% dplyr::filter(DAT %in% seq(as.Date(paste0(ani[j],"-08-01")), as.Date(paste0(ani[j+1],"-07-31")), "days"))
  tab.f10 <- rbind(tab.f10,t3)
  
}

Temp <- array(tab.f10$TMED, dim = c(nrow(tab.f10), 1))
Prec <- array(tab.f10$PRECIP, dim = c(nrow(tab.f10), 1))
elev <- array(tab.f10$Z)
TIME <- time_builder(time = tab.f10$DAT)
...
PARAMS <- expand.grid(SCF = 1, UADJ = seq(0.02,2,.2), MBASE = 0, MFMAX = 1.05, MFMIN = 0.60,TIPM = 0.2, NMF = 0.1, PLWHC = 0.04,
...
sSETTINGS <- expand.grid(RvS = 1, mDEC_RHO_COMPACTION = c(1), mDEC_RHO_DM_METAMORPH = 1, mDEC_RHO_NEW = 0, mDEC_RHO_METHOD = 0) 
...
tt <- tab.f10[c(1,2,3,4,5,6,7,12,16)]
df1 <- NULL
...
gg <- ggplot2::ggplot(df1)+
  geom_line(aes(x = DAT, y = SD, col = model))+
  geom_point(aes(x = DAT, y = GROSZ))
gg
gg1 <- ggplot2::ggplot(df1)+
  geom_line(aes(x = DAT, y = SWE, col = model))+
  geom_point(aes(x = DAT, y = ECHIVZAP))
gg1
